...
from eccgl1.AutoPACMEN_function import get_ec_number_kcats_wildcard_search
    ...
    * model: cobra.Model ~ A Model object which will be modified in place.
    ...
    for reaction in model.reactions:
        ...
        if reaction.lower_bound < 0 and reaction.upper_bound > 0:
            ...
            reaction_dict = {k: v * -1 for k, v in reaction.metabolites.items()}
    ...
    for r in model.reactions:
        ...
        if re.search(" and ", r.gene_reaction_rule):
            ...
            if eachgene in protein_id_mass_mapping.keys():
                ...
                reaction_mw[r.id] = (
                    protein_id_mass_mapping[eachgene] * enzyme_unit_number
                )
    json_write(json_output_file, reaction_mw)


# PUBLIC FUNCTIONS
def get_reaction_kcat_mw(
    model_file: str,
    project_folder: str,
    project_name: str,
    enzyme_unit_number_file: str,
    ...
    * type_of_default_kcat_selection: str ~ The type of selection of default kcat values. Can be "mean",
    ...
    for model_reaction_id in model_reaction_ids:
            ...
            if re.search(" and ", r.gene_reaction_rule):
                        ...
                        if enzyme_unit_number_file != "none":
                            if r.id in enzyme_unit_number.keys():
                                if eachgene in enzyme_unit_number[r.id].keys():
                                    mass_sum += protein_id_mass_mapping[eachgene] * int(
                                        enzyme_unit_number[r.id][eachgene]
                                    )
                        else:
                            # print(eachgene)
                ...
                if eachgene in protein_id_mass_mapping.keys():
                    ...
                    if enzyme_unit_number_file != "none":
                        if r.id in enzyme_unit_number.keys():
                            if eachgene in enzyme_unit_number[r.id].keys():
                                reaction_mw[r.id] = protein_id_mass_mapping[
                                    eachgene
                                ] * int(enzyme_unit_number[r.id][eachgene])
                    else:
        ...
        if model_reaction_id in reaction_mw.keys():
            ...
            reaction_kcat_mw[model_reaction_id]["kcat_MW"] = (
                reaction_kcat_mw[model_reaction_id]["kcat"]
                * 3600000
    ...
    reaction_kcat_mw_df_T_select = reaction_kcat_mw_df_T[
        abs(reaction_kcat_mw_df_T["kcat_MW"]) > 0
    ]
...
def trans_model2enz_json_model_split_isoenzyme(
    model_file,
    ...
    * upperbound:  Upperbound  of enzyme concentration constraint.
    ...
    for eachreaction in range(len(dictionary_model["reactions"])):
        ...
        if reaction_id in reaction_kcat_mw.index:
            dictionary_model["reactions"][eachreaction]["kcat"] = reaction_kcat_mw.loc[
                reaction_id, "kcat"
            ]
            dictionary_model["reactions"][eachreaction]["kcat_MW"] = (
                reaction_kcat_mw.loc[reaction_id, "kcat_MW"]
            )
        else:
...
def get_fluxes_detail_in_model(
            ...
            if index == eachreaction["id"]:
                if "annotation" in eachreaction.keys():
                    if "ec-code" in eachreaction["annotation"].keys():
                        if isinstance(eachreaction["annotation"]["ec-code"], list):
                            model_pfba_solution_detail.loc[index, "ec-code"] = (
                                ","
                            ).join(eachreaction["annotation"]["ec-code"])
                        else:
                            model_pfba_solution_detail.loc[index, "ec-code"] = (
                ...
                if "kcat_MW" in eachreaction.keys():
                    if eachreaction["kcat_MW"]:
                        model_pfba_solution_detail.loc[index, "kcat_MW"] = eachreaction[
                            "kcat_MW"
                        ]
                        model_pfba_solution_detail.loc[index, "E"] = float(
                            row["fluxes"]
                        ) / float(eachreaction["kcat_MW"])
                break
    ...
    for gene_i in gene_abundance.index:
        if gene_i in gene_mw.index:
            if isinstance(gene_mw.loc[gene_i, "Mass"], str):
                abundance = gene_abundance.loc[gene_i, gene_abundance_colname] * int(
                    gene_mw.loc[gene_i, "Mass"].replace(",", "")
                )
            else:
                abundance = gene_abundance.loc[gene_i, gene_abundance_colname] * int(
                    gene_mw.loc[gene_i, "Mass"][0].replace(",", "")
                )
...
def calculate_f_special(
    gene_abundance_file,
    modelgene2uniprot_file,
    paxdbgene2uniprot_file,
    gene_abundance_colname,
):
    """Calculating f (the mass fraction of enzymes that are accounted
    in the model out of all proteins) based on the protein abundance
    ...
    for gene_i in gene_abundance.index:
        ...
        if gene_i in list(paxdb_gene.index):
            ...
            if str(paxdb_gene.loc[gene_i, "Mass"]) != "nan":
                abundance = gene_abundance.loc[gene_i, gene_abundance_colname] * float(
                    paxdb_gene.loc[gene_i, "Mass"].replace(",", "")
                )
                # print(abundance)
                pro_abundance += abundance
...
def get_model_substrate_obj(use_model):
    ...
    EX_exclude_reaction_list = EX_exclude_reaction_list + [
        i + "_reverse" for i in EX_exclude_reaction_list
    ]
    for r in use_model.reactions:
        ...
        # elif not r.lower_bound==0 and not r.lower_bound==-1000 and not r.lower_bound==-999999 and abs(r.lower_bound)>0.1:#排除很小的值
            ...
            not r.upper_bound == 0
            and not r.upper_bound == 1000
            and not r.upper_bound == 999999
            and abs(r.upper_bound) > 0.1
        ):  # 排除很小的值
...
def get_protein_mass_mapping_from_local(
    sbml_path: str, project_folder: str, project_name: str, uniprot_data_file: str
) -> None:
...
def change_enz_model_by_enz_usage(
    enz_ratio,
    json_model_path,
    reaction_flux_file,
    EC_max_file,
    ...
    * reaction_mw_outfile: changed file stored reaction kcat/mw.
    ...
    reaction_fluxes = reaction_fluxes.sort_values(
    ...
    [need_change_reaction_list, changed_reaction_list, reaction_kcat_mw] = (
        adj_reaction_kcat_by_database(
            json_model_path,
            [select_reaction],
            need_change_reaction_list,
            changed_reaction_list,
...
def adj_reaction_kcat_by_database(
    json_model_path,
    select_reactionlist,
    need_change_reaction_list,
    changed_reaction_list,
        ...
        if "ec-code" in select_reaction.annotation.keys():
            ...
            if isinstance(ec_number, str):
                if ec_number in Brenda_sabio_combined_select.keys():
                    reaction_kcat_max = Brenda_sabio_combined_select[ec_number][
                        "kcat_max"
                    ]
                    if reaction_kcat_mw.loc[eachreaction, "kcat"] < reaction_kcat_max:
                        reaction_kcat_mw.loc[eachreaction, "kcat"] = (
                            reaction_kcat_max  # h_1
                        )
                        reaction_kcat_mw.loc[eachreaction, "kcat_MW"] = (
                            reaction_kcat_max
                            * 3600
                            * 1000
                ...
                for eachec in ec_number:
                    if eachec in Brenda_sabio_combined_select.keys():
                        kcat_max_list.append(
                            Brenda_sabio_combined_select[eachec]["kcat_max"]
                        )
...
def draw_cdf_fig(data_cdf_data, output_file, x_name, y_name, y_index, nticks):
    ...
    trace1 = go.Scatter(
        x=data_cdf_data, y=y_index, marker={"color": "blue"}, mode="lines"
    )
    data1 = [trace0, trace1]
    layout = go.Layout(
        plot_bgcolor="lightgrey",
        xaxis=dict(
            title=dict(text=x_name, font=dict(size=20, family="Times New Roman")),
            type="log",
            rangemode="tozero",
        ...
        yaxis=dict(
            title=dict(text=y_name, font=dict(size=20, family="Times New Roman")),
            range=[0, 1],
            showgrid=False,
            zeroline=False,
            rangemode="tozero",
...
def draw_cdf_fig_kcat(data_cdf_data, output_file, x_name, y_name, y_index, nticks):
    trace0 = go.Scatter(
        x=data_cdf_data, y=y_index, mode="lines", marker={"color": "blue"}
    )
    trace1 = go.Scatter(
        x=data_cdf_data,
        y=y_index,
    ...
    * dict_coeff: {reaction ID: coeffient}.
    ...
    with model:
        ...
        cons_obj = model.problem.Constraint(
            model.objective.expression, lb=min(bounds), ub=max(bounds)
        )
...
def draw_3d_rbas(z_data, out_fig_file):
    ...
    fig = go.Figure(
        data=[go.Surface(y=exlist, x=exlisto2, z=z_data.values)], layout=layout
    )
    fig.update_layout(
        scene=dict(
            xaxis=dict(
                ...
                title=dict(
                    text="<b>O2 uptake rates<br>(mmol/gDW/h)</b>",
            ...
            yaxis=dict(
                ...
                backgroundcolor="lightgrey",
                title=dict(
                    text="<b>Glucose uptake rates<br>(mmol/gDW/h)</b>",
                    font=dict(size=18, family="Times New Roman"),
                ),
            ),
            zaxis=dict(
    ...
    fig.update_traces(
        contours_z=dict(usecolormap=True, highlightcolor="mistyrose", project_z=True)
    )
